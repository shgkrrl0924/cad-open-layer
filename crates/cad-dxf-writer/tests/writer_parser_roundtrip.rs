...
fn round_trip_basic_entities() {
    let entities = vec![
        ...
        line(Point::new(10000.0, 0.0, 0.0), Point::new(10000.0, 6500.0, 0.0), "WALLS"),
    ...
    assert_eq!(parsed.header.get("$ACADVER").map(String::as_str), Some("AC1015"));
    ...
    if let Entity::Line { p1, p2, layer, .. } = &parsed.entities[0] {
        ...
        panic!("first entity should be a LINE, got {:?}", parsed.entities[0]);
    }

    // Verify ARC angles preserved (degrees → radians → degrees).
    if let Entity::Arc { radius, start_angle, end_angle, layer, .. } =
        &parsed.entities[2]
    {
        ...
        panic!("third entity should be an ARC, got {:?}", parsed.entities[2]);
    }
...
fn round_trip_with_block_definitions() {
    ...
    let block = BlockDef {
        ...
        flags: 0,
        entities: vec![
            line(Point::new(0.0, 0.0, 0.0), Point::new(800.0, 0.0, 0.0), "DOORS"),
    ...
    let user_blocks: Vec<_> = parsed.blocks.iter().filter(|b| !b.name.starts_with('*')).collect();
    assert_eq!(user_blocks.len(), 1, "should round-trip 1 user BLOCK definition");
    assert_eq!(user_blocks[0].name, "DOOR_800");
    assert_eq!(user_blocks[0].entities.len(), 2);

    if let Entity::Insert { block_name, position, .. } = &parsed.entities[0] {
...
fn raw_extras_survive_round_trip() {
    ...
    assert!(
        has_marker,
        "raw_extras (999) must survive write/re-parse, got {:?}",
        extras
    );
}
